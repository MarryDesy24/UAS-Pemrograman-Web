...
function GuruKelasView() {
  ...
  const handleDeleteClass = async () => {
    if (!selectedKelas) return;
    if (!confirm(`Yakin ingin menghapus kelas "${selectedKelas.nama}"? Semua data siswa di kelas ini juga akan dihapus.`)) return;

    const { error } = await supabase.from('classrooms').delete().eq('id', selectedKelas.id);
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleDeleteClass} className="px-4 py-2 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition-colors text-sm flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
